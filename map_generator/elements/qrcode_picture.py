try:
    ...
    import sys
    import os        
    path = os.path.join(os.path.dirname(__file__),'qrcode','qrcode-6.1-py2.py3-none-any.whl')
    path = 'C:/Users/apotheosis/AppData/Roaming/QGIS/QGIS3/profiles/default/python/plugins/edition_plugin/map_generator/elements/qrcode/qrcode-6.1-py2.py3-none-any.whl'
...
def createQRCode(dest_path, latitude, longitude, camadas_adicionar, escala, nivel_de_zoom = None):
    ...
    # Niveis de zoom    
    if nivel_de_zoom is None:
        nivel_de_zoom = niveis_de_zoom[escala]

    # https://bdgex.eb.mil.br/bdgex/mobile/?l=963,953&c=-53.708451,-26.680751&z=10
    # Substituindo os dados
    original_path = "https://bdgex.eb.mil.br/bdgex/mobile/?l={layers}&c={longitude},{latitude}&z={nivel_de_zoom}"
...
def create_qrcode_from_feature(selected_feature, escala, camadas_adicionar, inom ):
    ...
    latitude = centroid_pt.y()
    longitude = centroid_pt.x()    
    #dest_folder = os.path.join(os.path.dirname(os.path.dirname(os.path.dirname(__file__))),'includes','output','temporary')
    dest_folder = os.path.join(os.path.dirname(os.path.dirname(os.path.dirname(__file__))),'includes','output','temporary')   
    path = os.path.join(os.path.dirname(__file__),'qrcode','qrcode-6.1-py2.py3-none-any.whl')
    dest_path = os.path.join(dest_folder, inom + ".png")
...
def replace_qrCode(composition, path_qrCode, idPicture_qrCode="symbol_QRCODE"):
    if composition.itemById(idPicture_qrCode) is not None:
        ...
        picture_QRCode.setPicturePath(path_qrCode)
        picture_QRCode.refresh()

def delete_file(caminho_para_remover):
    if os.path.exists(caminho_para_remover):
        os.remove(caminho_para_remover)
