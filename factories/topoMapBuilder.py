...
class TopoMapBuilder(IMapBuilder, MapBuilderUtils):
    def __init__(self, componentFactory: ComponentFactory) -> None:
        ...
        self.components.update(
            {"map": self.componentFactory.getComponent("Map", "topoMap")}
        )
        self.components.update(
    ...
    def run(self, debugMode: bool = False):
        ...
        elevationDiagramLayers, elevationDiagramLayersIds = self.getLayersFromDB(
            self.conn,
            self.data,
            self.defaults,
            self.productPath,
