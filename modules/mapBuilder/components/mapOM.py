...
class MapOM(ComponentUtils, IComponent):
    def __init__(self, *args, **kwargs):
        self.stylesFolder = (
            Path(__file__).parent.parent / "resources" / "products" / "omMap" / "styles"
        )
        self.defaultMapSize = [
            (1025, 865),
            (550, 550),
    ...
    def build(
        ...
        utmGeom = self.transformGeometry(
            mapAreaFeature.geometry(), "EPSG:4674", f'EPSG:{jsonData.get("epsg")}'
        )
        obb_native = processing.run(
            "native:orientedminimumboundingbox",
            {"INPUT": mapAreaLayer, "OUTPUT": "TEMPORARY_OUTPUT"},
        )

        mapAreaFeaureAdjustPlacement = next(
            obb_native["OUTPUT"].getFeatures()
        ).geometry()
        mapAreaFeaureAdjustPlacement = self.transformGeometry(
            mapAreaFeaureAdjustPlacement, "EPSG:4674", f'EPSG:{jsonData.get("epsg")}'
        )
        mapAreaFeaureAdjustPlacement = QgsRectangle().fromWkt(
            mapAreaFeaureAdjustPlacement.asWkt()
        )
    ...
    def createOmLayer(
        self, baseLayer: QgsVectorLayer, mapAreaFeature: QgsFeature, data: dict
    ) -> QgsVectorLayer:
    ...
    def updateComposition(
        self,
        composition: QgsPrintLayout,
        omLayer: QgsVectorLayer,
        layersToComposition: list[QgsMapLayer],
        data: dict,
        mapAreaExtentsAfterRotation: QgsRectangle,
    ):
