...
class MiniMapCoords(ComponentUtils, IComponent):
    ...
    def build(self, composition: QgsPrintLayout, mapAreaFeature: QgsFeature):
        ...
        minimap_points = self.customgetMiniMaPointsCoordinates(
            mapAreaFeature, minimap_layer
        )
    ...
    def getMiniMaPointsCoordinates(self, miniMapLayer):
        ...
        pontos_inferiores, pontos_superiores = sorted(
            orderedy_minimap_points[0:2], key=lambda k: k["x"]
        ), sorted(orderedy_minimap_points[2:], key=lambda k: k["x"])
        ponto_superior_esquerdo, ponto_superior_direito = (
            pontos_superiores[0],
            pontos_superiores[1],
        )
