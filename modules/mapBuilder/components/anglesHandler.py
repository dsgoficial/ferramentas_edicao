...
class AnglesHandler:
    ...
    def generateDMS(self, ang):
        ...
        gms = str(xg) + "° " + str(xm).zfill(2) + "' " + str(xs).zfill(2) + '"'
        gms = gms.encode("utf8")
        return gms.decode("utf8")
    ...
    def updateComposition(
        ...
        convergencia,
        declinacao,
        yearlydelta_declination,
    ):
        # Yearly declination delta
        label_yearlyDeclinationDelta = composition.itemById(
            "label_yearlyDeclinationDelta"
        )
        ...
        if magneticDeclLabel := composition.itemById("label_declinacao_ano"):
            magneticDeclLabel.setText(
                f"DECLINAÇÃO MAGNÉTICA EM 1° DE JANEIRO DE {datetime.datetime.now().year} E CONVERGÊNCIA MERIDIANA DO CENTRO DO PRODUTO"
            )
        ...
        for compositionItem in composition_list:
            ...
            check_polyline = (
                compositionItem
            ).__class__.__name__ == "QgsLayoutItemPolyline"
            check_picture = (
                compositionItem
            ).__class__.__name__ == "QgsLayoutItemPicture"
