...
from qgis.core import   (QgsProject, QgsFeature, QgsFeatureIterator, QgsFeatureRequest,
                         QgsGeometry, QgsDistanceArea, QgsPoint,
                         QgsExpression, QgsExpressionContext, QgsExpressionContextScope,
                         QgsPalLayerSettings,
                         QgsFields, QgsField,
                         QgsCoordinateReferenceSystem, QgsCoordinateTransformContext,
                         QgsSpatialIndex, QgsDistanceArea, QgsUnitTypes,
                         NULL )
...
class CopySugestedLabel(QgsMapToolEmitPoint, BaseTools):
    ...
    def setupUi(self):
        ...
        self._action = self.createAction(
            ...
            None,
            lambda _: None,
            self.tr('Copia rótulos sugeridos pelo QGIS na camada ativa para "edicao_texto_generico_l"'),
            self.tr('Copia rótulos sugeridos pelo QGIS na camada ativa para "edicao_texto_generico_l"'),
            self.iface
        )
    ...
    def getAcceptedRule(self, baseLabelRule, context,configs = []):
        if isinstance(baseLabelRule, list):
            for childLabelRule in baseLabelRule:
                    ...
                    if rule_expresion.evaluate(context):                    
            ...
            if n_children==0:
                ...
                if rule_expresion.evaluate(context)  or rule_expresion.expression()=='':
                    ...
                    if rule_settings is not None:
                        ...
                        # Get custom values
                        checkExpressionProperties = {
                            'tamanho_txt': QgsPalLayerSettings.Size
                        }                    
                        for attr, prop in checkExpressionProperties.items():
                            if prop in rule_settings.dataDefinedProperties().propertyKeys():
                                if rule_settings.dataDefinedProperties().property(prop).isActive():                                                                
                                    prop_expression = QgsExpression(rule_settings.dataDefinedProperties().property(prop).asExpression())
    ...
    def getSugestedLabelGeometry(self, letters_data, custom = None):
        ...
        word_geometry = QgsGeometry.fromPolyline(words_points)
        oldLen = word_geometry.length()
        word_geometry = word_geometry.extendLine(oldLen*0.05, oldLen*0.05)
    ...
    def mouseClick(self, pos, btn):
        ...
        # Make label index
        self.labelSpatialIndex = QgsSpatialIndex( flags=QgsSpatialIndex.FlagStoreFeatureGeometries)
        ...
        if self.isActive() and self.dstLyr:                        
            labelClosestSpatialID = self.labelSpatialIndex.nearestNeighbor(pos, maxDistance=2*self.tolerance)
            label_request = QgsFeatureRequest().setFilterFids(labelClosestSpatialID)
            closestSpatialID =  False if len(labelClosestSpatialID)==0 else labelClosestSpatialID[0]
            ...
            if closestSpatialID:
                ...
                if self.checkFeature(feat):
                    ...
                    sugestedLabelGeometry, word_text = self.getSugestedLabelGeometry([(label_feature.geometry(),label_feature['labelText']) for label_feature in self.letter_label_features if label_feature.id() == closestSpatialID])                
                    sugestedLabelConfig = self.getSugestedLabelConfig(feat)
                    if sugestedLabelConfig['found']:
                        self.createSugestedLabelFeature(feat,word_text, sugestedLabelGeometry, sugestedLabelConfig)                    
                    else:
                        self.displayErrorMessage(self.tr(
                            'Feição inválida. Verifique as configurações do rótulo.'
                        ))
                else:
                    self.displayErrorMessage(self.tr(
                        'Feição inválida. Verifique os atributos na camada ativa.'
                    ))
            else:
    ...
    def createSugestedLabelFeature(self, originFeat, word_text, sugestedLabelGeometry, sugestedLabelConfig):
    ...
    def getLayers(self):                
        if not (lyr:=self.iface.activeLayer()):
            ...
            if fieldIdx == -1:
                self.displayErrorMessage(self.tr('O atributo "texto_edicao" não existe na camada selecionada'))
            else:
