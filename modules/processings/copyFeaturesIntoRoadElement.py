...
class CopyFeaturesIntoRoadElement(QgsProcessingAlgorithm):
    ...
    def processAlgorithm(self, parameters, context, feedback):
        ...
        railways_layer = self.parameterAsVectorLayer(parameters, self.INPUT_RAILWAYS, context)
        ditches_layer = self.parameterAsVectorLayer(parameters, self.INPUT_DITCHES, context)
        drainage_layer = self.parameterAsVectorLayer(parameters, self.INPUT_DRAINAGE, context)
        road_element_layer = self.parameterAsVectorLayer(parameters, self.OUTPUT_ROAD_ELEMENT, context)
    ...
    def process_roads(self, layer, target_layer, feedback):
        try:
            ...
            if total == 0:
                feedback.pushInfo(self.tr("Nenhuma feição de rodovia encontrada para copiar."))
                return
            ...
            for current, feature in enumerate(features):
                ...
                new_feature = QgsVectorLayerUtils.createFeature(target_layer, feature.geometry())
                ...
                if "posicao_pista_elemento_viario" in fields:
                    new_feature["posicao_pista"] = feature["posicao_pista_elemento_viario"]
                
                if "material_construcao_elemento_viario" in fields:
                    new_feature["material_construcao"] = feature["material_construcao_elemento_viario"]
            ...
            feedback.pushInfo(self.tr(f"Adicionadas {len(new_features)} feições de rodovias."))
    ...
    def process_railways(self, layer, target_layer, feedback):
        try:
            ...
            if total == 0:
                feedback.pushInfo(self.tr("Nenhuma feição de ferrovia encontrada para copiar."))
                return
            ...
            feedback.pushInfo(self.tr(f"Adicionadas {len(new_features)} feições de ferrovias."))
    ...
    def process_ditches(self, layer, target_layer, feedback):
        try:
            ...
            if total == 0:
                feedback.pushInfo(self.tr("Nenhuma feição de vala encontrada para copiar."))
                return
            ...
            feedback.pushInfo(self.tr(f"Adicionadas {len(new_features)} feições de valas."))
    ...
    def process_drainage(self, layer, target_layer, feedback):
        try:
            ...
            if total == 0:
                feedback.pushInfo(self.tr("Nenhuma feição de trecho de drenagem encontrada para copiar."))
                return
            
            feedback.pushInfo(self.tr(f"Copiando {total} feições de trechos de drenagem..."))
            ...
            feedback.pushInfo(self.tr(f"Adicionadas {len(new_features)} feições de trechos de drenagem."))
            
        except Exception as e:
            feedback.reportError(self.tr(f"Erro ao processar trechos de drenagem: {str(e)}"))
