...
class LayoutItemsToCanvasAlgorithm(QgsProcessingAlgorithm):
    ...
    def initAlgorithm(self, config=None):
        self.addParameter(
            QgsProcessingParameterLayout(self.INPUT_LAYOUT, self.tr("Input layout"))
        )
        self.addParameter(
            QgsProcessingParameterLayoutItem(
                self.REFERENCE_MAP,
                self.tr("Mapa de referência"),
    ...
    def convert_layout_to_map(self, layout_x, layout_y, layout_rect, map_extent):
        ...
        scale_x = (map_extent.xMaximum() - map_extent.xMinimum()) / (
            layout_rect.width()
        )
        scale_y = (map_extent.yMaximum() - map_extent.yMinimum()) / (
            layout_rect.height()
        )

        map_x = map_extent.xMinimum() + (layout_x - layout_rect.x()) * scale_x
        map_y = (
            map_extent.yMaximum() - (layout_y - layout_rect.y()) * scale_y
        )  # Inverter eixo Y
    ...
    def get_item_bounds(self, item, main_map, layout):
        ...
        map_top_left = self.convert_layout_to_map(
            scene_rect.left(), scene_rect.top(), layout_rect, map_extent
        )
        map_bottom_right = self.convert_layout_to_map(
            scene_rect.right(), scene_rect.bottom(), layout_rect, map_extent
        )
    ...
    def get_shape_geometry(self, item, main_map, layout):
        ...
        points = [
            self.convert_layout_to_map(
                scene_rect.left(), scene_rect.top(), layout_rect, map_extent
            ),
            self.convert_layout_to_map(
                scene_rect.right(), scene_rect.top(), layout_rect, map_extent
            ),
            self.convert_layout_to_map(
                scene_rect.right(), scene_rect.bottom(), layout_rect, map_extent
            ),
            self.convert_layout_to_map(
                scene_rect.left(), scene_rect.bottom(), layout_rect, map_extent
            ),
            self.convert_layout_to_map(
                scene_rect.left(), scene_rect.top(), layout_rect, map_extent
            ),  # Fechar o polígono
    ...
    def get_map_bounds_geometry(self, main_map, layout):
        ...
        points = [
            self.convert_layout_to_map(
                layout_rect.left(), layout_rect.top(), layout_rect, map_extent
            ),
            self.convert_layout_to_map(
                layout_rect.right(), layout_rect.top(), layout_rect, map_extent
            ),
            self.convert_layout_to_map(
                layout_rect.right(), layout_rect.bottom(), layout_rect, map_extent
            ),
            self.convert_layout_to_map(
                layout_rect.left(), layout_rect.bottom(), layout_rect, map_extent
            ),
            self.convert_layout_to_map(
                layout_rect.left(), layout_rect.top(), layout_rect, map_extent
            ),  # Fechar o polígono
    ...
    def processAlgorithm(self, parameters, context, feedback):
        ...
        reference_map = self.parameterAsLayoutItem(
            parameters, self.REFERENCE_MAP, context, layout
        )
        ...
        main_map = next(
            (
                ...
                if isinstance(item, QgsLayoutItemMap) and item.id() == reference_map_id
            ),
            None,
        )
        ...
        if not main_map:
            feedback.reportError(
                f'Error: Map with ID "{reference_map_id}" not found in layout.',
                fatalError=True,
            )
            return {}
        ...
        for item in layout.items():
            ...
            if not self.should_process_item(item):
                continue
            feedback.pushInfo(
                f"Processing item: {item.id()} (Type: {type(item).__name__})"
            )
            ...
            elif isinstance(item, QgsLayoutItemPicture) or isinstance(
                item, QgsLayoutItemLabel
            ):
            ...
            if features_added > 1000:
                feedback.reportError(
                    "Too many elements processed. Stopping for performance reasons.",
                    fatalError=True,
                )
