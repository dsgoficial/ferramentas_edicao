...
class MergeHighway(QgsProcessingAlgorithm):
    ...
    def initAlgorithm(self, config=None):
        ...
        self.addParameter(
            QgsProcessingParameterFeatureSink(
                self.OUTPUT_LAYER_L, self.tr("rodovia_mesclada")
            )
        )

    def processAlgorithm(self, parameters, context, feedback):
        highwayLayerInput = self.parameterAsVectorLayer(
    ...
    def shortHelpString(self):
        return self.tr(
            "Mescla as rodovias de acordo com o atributo *sigla*, desconsiderando rodovias sem sigla, e depois recorta as rodovias com a moldura. O resultado é retornado em outra camada e é utilizado como referência para auxiliar no processo de edição."
        )
