...
from qgis.core import (QgsCoordinateReferenceSystem, QgsFeature,
                       ...
                       QgsWkbTypes, QgsFields, QgsVectorLayerUtils, QgsDistanceArea)
...
class ElevationDiagramPointGeneralization(QgsProcessingAlgorithm): 
    ...
    def initAlgorithm(self, config=None):
        ...
        self.addParameter(
            QgsProcessingParameterEnum(
                'INPUT_SCALE',
                self.tr('Selecione a escala para gerar as quadriculas do diagrama de elevação'),
    ...
    def processAlgorithm(self, parameters, context, feedback):      
        ...
        inputPointsLyr = self.parameterAsVectorLayer(parameters, self.INPUT_ELEVATION_POINTS, context)
    ...
    def generalizePoints(self, inputPointsLyr, elevationField, grid, geographicBoundaryLyr, minDistance, context, feedback):
        ...
        geographicBoundaryGeometry = [feat.geometry() for feat in dissolvedGeographicBoundaryLyr.getFeatures()][0]
        ...
        for current, gridPolygonFeat in enumerate(grid.getFeatures()):
            ...
            bufferSize = 0.05 * QgsDistanceArea().measurePerimeter(geom) / 4 # buffer de 5% do comprimento do lado original
            bufferGeom = geom.buffer(-bufferSize, -1)
            if not geographicBoundaryGeometry.intersects(bufferGeom):
                continue
            pointFeats = [
                feat for feat in cachedPoints.getFeatures(
                    bufferGeom.boundingBox()
            ...
            if len(pointFeats) == 0:
                ...
                if len(pointFeats) == 0:
                    continue
            selectedFeat = list(sorted(pointFeats, key=lambda x: x[elevationField], reverse=True))[0]
    ...
    def shortHelpString(self):
        return self.tr("O algoritmo generaliza os pontos cotados para o diagrama de elevação")
