...
class GeneralizeBuildings(QgsProcessingAlgorithm):
    ...
    def initAlgorithm(self, config=None):
        ...
        self.addParameter(
            core.QgsProcessingParameterField(
                ...
                self.tr(
                    'Selecionar o atributo de "visibilidade" da camada de edificação'
                ),
        ...
        self.addParameter(
            core.QgsProcessingParameterField(
                ...
                self.tr(
                    'Selecionar o atributo de "visibilidade" da camada de depósito'
                ),
                type=core.QgsProcessingParameterField.Any,
    ...
    def processAlgorithm(self, parameters, context, feedback):
        buildingLayer = self.parameterAsVectorLayer(
            parameters, self.INPUT_POINT_BUILDING, context
        )
        buildingVisibleField = self.parameterAsFields(
            parameters, self.INPUT_BUILDING_VISIBLE_FIELD, context
        )[0]
        depositLayer = self.parameterAsVectorLayer(
            parameters, self.INPUT_POINT_WAREHOUSE, context
        )
        depositVisibleField = self.parameterAsFields(
            parameters, self.INPUT_POINT_WAREHOUSE_VISIBLE_FIELD, context
        )[0]
        mineralExtractionLayer = self.parameterAsVectorLayer(
            parameters, self.INPUT_MINERAL_EXTRACTION_A, context
        )
        buildingArea = self.parameterAsVectorLayer(
            parameters, self.INPUT_BUILDING_AREA_A, context
        )
        ...
        for mineralExtractionFeature in mineralExtractionLayer.getFeatures():
            ...
            request = QgsFeatureRequest().setFilterRect(
                mineralExtractionGeometry.boundingBox()
            )
            for visibleField, layer in [
                (buildingVisibleField, buildingLayer),
                (depositVisibleField, depositLayer),
            ]:
        ...
        for buildingAreaFeature in buildingArea.getFeatures():
            ...
            request = QgsFeatureRequest().setFilterRect(
                buildingAreaGeometry.boundingBox()
            )
            for visibleField, layer in [(buildingVisibleField, buildingLayer)]:
                ...
                for feature in features:
                    if not (
                        feature["tipo"] == 0
                        and feature.geometry().intersects(buildingAreaGeometry)
                    ):
                        continue
                    feature[visibleField] = 2
                    self.updateLayerFeature(layer, feature)
