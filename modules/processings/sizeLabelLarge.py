...
class SizeLabelLarge(QgsProcessingAlgorithm):
    ...
    def processAlgorithm(self, parameters, context, feedback):
        inputPolygonLyr = self.parameterAsVectorLayer(parameters, self.INPUT_POLYGON, context)
        pointLyr = self.parameterAsVectorLayer(parameters, self.INPUT_POINT_TEXT, context)
        lineLyr = self.parameterAsVectorLayer(parameters, self.INPUT_LINE_TEXT, context)
        if pointLyr is None and lineLyr is None:
            raise QgsProcessingException("Escolha pelo menos uma camada de texto genérico")
        inputList = [pointLyr, lineLyr]
        inputList = list(filter(lambda x: x is not None, inputList))
        inputLyrSizeTextField = self.parameterAsFields(parameters, self.SIZE_TEXT, context)[0]
        ...
        for idx, lyr in enumerate(inputList):
            ...
            for current, pointFeature in enumerate(lyr.getFeatures()):
                ...
                for current1, polygonId in enumerate(polygonSpatialIdx.intersects(bboxPoint)):
                    ...
                    if geomPoint.within(geomPolygon):
                        ...
                        self.updateSizeText(inputPolygonLyr, lyr, inputLyrSizeTextField, pointFeature, geomPolygon, gridScaleParam)
                        break
                    else:
                ...
                for current2, (idDictPolygon, featDictPolygon) in enumerate(dictPolygon.items()):
                ...
                self.updateSizeText(inputPolygonLyr, lyr, inputLyrSizeTextField, pointFeature, geomNearstPolygon, gridScaleParam)
    ...
    def updateSizeText(self, inputPolygonLyr, inputPointLyr, inputLyrSizeTextField, pointFeature, geomPolygon, gridScaleParam):
        pointFeature[inputLyrSizeTextField] = self.sizeOfText(inputPolygonLyr, geomPolygon, gridScaleParam)
    ...
    def computeSearchBuffer(self, inputPointLyr, gridScaleParam):
        ...
        return d.convertLengthMeasurement(
            d_meters, QgsUnitTypes.DistanceDegrees
        )
    ...
    def sizeOfText (self, inputPolygonLyr, geomNearstPolygon, gridScaleParam):
        ...
        if crs.isGeographic():
            ...
            areaPolygon = 1e6 * convertForMili.convertAreaMeasurement(areaPolygon, QgsUnitTypes.AreaSquareMeters) / (scale**2)
            diamCircle = 1e3 * convertForMili.convertLengthMeasurement(diamCircle, QgsUnitTypes.DistanceMeters) / (scale)
    ...
    def shortHelpString(self):
        return self.tr("O algoritmo calcula o tamanho do texto de acordo com a área e a largura do polígono")
