...
class PlaceAllAreaPointSymbols(QgsProcessingAlgorithm): 
    ...
    def initAlgorithm(self, config=None):
        ...
        self.addParameter(
            QgsProcessingParameterBoolean(
                self.ONLY_SELECTED,
                self.tr('Executar somente nas feições selecionadas de todas as camadas de entrada')
            )
        )
        self.addParameter(
            QgsProcessingParameterBoolean(
    ...
    def processAlgorithm(self, parameters, context, feedback):
        inputAreaBuildingsLyr = self.parameterAsVectorLayer(parameters, self.INPUT_AREA_BUILDINGS, context)
        inputPowerPlantLyr = self.parameterAsVectorLayer(parameters, self.INPUT_POWER_PLANT, context)
        inputMineralExtractionLyr = self.parameterAsVectorLayer(parameters, self.INPUT_MINERAL_EXTRACTION, context)
        inputLandUseLyr = self.parameterAsVectorLayer(parameters, self.INPUT_LAND_USE, context)
        ...
        multiStepFeedback.setProgressText("Executando o posicionamento dos símbolos de edificações área no ponto médio do segmento mais ao norte")
        processing.run(
        ...
        multiStepFeedback.setProgressText("Executando o posicionamento dos símbolos de ocupação do solo")
        processing.run(
        ...
        multiStepFeedback.setProgressText("Executando o posicionamento do símbolo de hospitais")
        processing.run(
        ...
        multiStepFeedback.setProgressText("Executando o posicionamento dos símbolos de subestação de energia")
        processing.run(
        ...
        multiStepFeedback.setProgressText("Executando o posicionamento dos símbolos de extração mineral")
        processing.run(
            "ferramentasedicao:placepowerplantsymbol",
            {
